...
current_date = st.sidebar.date_input("Start Date", datetime.today())
...
for i in list_stk:
	...
	df["Signal"]=np.where((((df.stochrsi!="-") & (df.CPrice < df.MA) & (df.Previous > 80) & (df.Current < 80)) | ((df.stochrsi!="-") & (df.CPrice > df.MA) & (df.Previous < 20) & (df.Current > 20))),"yes","-")
